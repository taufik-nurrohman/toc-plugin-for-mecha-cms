<form class="form-plugin" action="<?php $toc_config = File::open(PLUGIN . DS . 'toc' . DS . 'states' . DS . 'config.txt')->unserialize(); echo $config->url_current; ?>/update" method="post">
  ...
  <label class="grid-group">
    ...
    <span class="grid span-4"><input name="toc_back_title" type="text" class="input-block" value="<?php echo Text::parse($toc_config['toc_back_title'])->to_encoded_html; ?>"></span>
  </label>
  <label class="grid-group">
    <span class="grid span-2 form-label"><?php echo $speak->plugin_toc_title_prefix; ?></span>
    <span class="grid span-4"><input name="id_prefix" type="text" value="<?php echo $toc_config['id_prefix']; ?>" placeholder="<?php echo $speak->plugin_toc_placeholder_prefix; ?>"></span>
  </label>
  <label class="grid-group">
    <span class="grid span-2 form-label"><?php echo $speak->plugin_toc_title_suffix; ?></span>
    <span class="grid span-4"><input name="id_suffix" type="text" value="<?php echo $toc_config['id_suffix']; ?>" placeholder="<?php echo $speak->plugin_toc_placeholder_suffix; ?>"></span>
  </label>
